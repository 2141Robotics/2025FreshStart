...
/**
 * The ElevatorArm subsystem manages a two-motor elevator and a single-motor arm for game piece
 * manipulation.
 *
 * <p>Hardware Configuration: - Elevator: Two TalonFX motors (one leader, one follower) - Arm: One
 * TalonFX motor
 *
 * <p>The subsystem provides commands for: - Manual control (joystick-based) - Position control
 * (preset positions) - Sequence commands (coordinated movements)
 */
public class ElevatorArm extends SubsystemBase {
   ...
   *
   * <p>This method should be called once during robot initialization. If called multiple times,
   * subsequent calls will be ignored with a warning.
   */
  public void init() {
    if (isInitialized) {
      System.err.println(
          "WARNING: ElevatorArm subsystem already initialized. Ignoring duplicate initialization.");
      return;
    }
  ...
  private void configureElevatorMotors() {
    SoftwareLimitSwitchConfigs elevatorLimits =
    ...
    MotorOutputConfigs leftMotorOutput =
  ...
  private void configureArmMotor() {
    SoftwareLimitSwitchConfigs armLimits =
        new SoftwareLimitSwitchConfigs()
            .withForwardSoftLimitEnable(true)
    ...
    MotorOutputConfigs armOutput =
        new MotorOutputConfigs().withInverted(InvertedValue.CounterClockwise_Positive);

    CurrentLimitsConfigs armCurrentLimits = new CurrentLimitsConfigs();

    FeedbackConfigs armFeedback =
        new FeedbackConfigs().withSensorToMechanismRatio(Constants.ARM_GEAR_RATIO);
    ...
    armConfig.OpenLoopRamps =
        new OpenLoopRampsConfigs().withDutyCycleOpenLoopRampPeriod(Constants.ARM_TIME_TO_MAX_SPEED);
    armConfig.ClosedLoopRamps =
        new ClosedLoopRampsConfigs()
  ...
  public void initSendable(SendableBuilder builder) {
    ...
    builder.addDoubleProperty(
        ...
        null);
    builder.addDoubleProperty(
        "Elevator Velocity (RPS)", () -> this.leftMotor.getVelocity().getValueAsDouble(), null);
    builder.addDoubleProperty(
        "Elevator Voltage (V)", () -> this.leftMotor.getMotorVoltage().getValueAsDouble(), null);

    builder.addDoubleProperty(
        "Arm Position (Rotations)", () -> this.armMotor.getPosition().getValueAsDouble(), null);
    builder.addDoubleProperty(
        "Arm Velocity (RPS)", () -> this.armMotor.getVelocity().getValueAsDouble(), null);
    builder.addDoubleProperty(
        "Arm Voltage (V)", () -> this.armMotor.getMotorVoltage().getValueAsDouble(), null);
  }
  ...
  public Command stopElevatorCommand() {
    return this.runOnce(this::stopElevator).withName("StopElevator");
  }

  public Command stopArmCommand() {
    return this.runOnce(this::stopArm).withName("StopArm");
  }
  ...
  public Command elevatorUp() {
    return setElevatorManualSpeed(Constants.ELEVATOR_MANUAL_SPEED).withName("ElevatorUp");
  }

  public Command elevatorDown() {
    return setElevatorManualSpeed(Constants.ELEVATOR_MANUAL_DOWN_SPEED).withName("ElevatorDown");
  }

  public Command armUp() {
    return setArmManualSpeed(Constants.ARM_MANUAL_UP_SPEED).withName("ArmUp");
  }

  public Command armDown() {
    return setArmManualSpeed(Constants.ARM_MANUAL_DOWN_SPEED).withName("ArmDown");
  }
        ...
        .until(
            () ->
                this.armMotor.getPosition().getValueAsDouble()
                    >= preset.getPosition() - Constants.ARM_TOLERANCE)
  ...
  public Command waitForElevatorAbove(ElevatorPreset preset) {
    return this.run(() -> {})
        .until(
            () ->
                this.leftMotor.getPosition().getValueAsDouble()
                    >= preset.getPosition() - Constants.ELEVATOR_TOLERANCE)
  ...
  private Command scoringSequence(ElevatorPreset elevatorPreset, ArmPreset armPreset) {
    return Commands.sequence(
            ...
            setElevatorPosition(elevatorPreset))
        .withName(elevatorPreset.name() + "Sequence");
  }
